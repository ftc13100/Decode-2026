...
import org.firstinspires.ftc.teamcode.vision.DualColorBallDetectionPipeline
...
@TeleOp(name = "DualColorBallVisionDemo", group = "Vision")
class DualColorBallVisionDemo : OpMode() {

    private lateinit var webcam: OpenCvWebcam
    private lateinit var pipeline: DualColorBallDetectionPipeline
    ...
    override fun init() {
        ...
        pipeline = DualColorBallDetectionPipeline(
            lowerHsvColor1 = Scalar(35.0, 50.0, 50.0),
            upperHsvColor1 = Scalar(85.0, 255.0, 255.0),
            lowerHsvColor2 = Scalar(125.0, 50.0, 50.0),
            upperHsvColor2 = Scalar(165.0, 255.0, 255.0),
            minArea = 250.0,
        )
    ...
    override fun loop() {
        ...
        val ballsSnapshot = synchronized(pipeline.balls) {
            pipeline.balls.map { Triple(it.center, it.color, it.area) }
        }
        val firstBall = ballsSnapshot.firstOrNull()

        if (firstBall != null && !follower.isBusy) {
            val targetPose = pixelToField(firstBall.first, pipeline.frameWidth, pipeline.frameHeight)
        ...
        telemetry.addData("Total Balls", ballsSnapshot.size)
        ballsSnapshot.forEach { (center, color, area) ->
            telemetry.addData("$color", "(%.1f, %.1f) area=%.0f", center.x, center.y, area)
        }

...
object GreenBallCounter : Subsystem {

    private const val GREEN_COUNT_PIPELINE = 7
    private const val SMOOTHING_WINDOW = 5
    private const val EMPTY_FRAME_HOLD = 8
    private const val MAX_COUNT = 6

    // Tune this upward if green keeps undercounting.
    @JvmField
    var ratioGain = 1.08
    ...
    override fun initialize() {
        ...
        limelight.pipelineSwitch(GREEN_COUNT_PIPELINE)
        limelight.start()
    }
    ...
    private val updateCountCommand = InstantCommand {
        ...
        if (result != null) {
            ...
            if (blobs.isEmpty()) {
                ...
                emptyFrameStreak = 0
                // Blob count is a lower bound when corners are missing.

...
class redBottom: NextFTCOpMode() {
    ...
    private val shootPose = Pose(88.0, 8.0, Math.toRadians(0.0)).mirror()
    //path to pick up PPG motif
    private val pickUpPPG = Pose(127.7, 83.0, Math.toRadians(0.0)).mirror()
    private val pickUpPPGcontrol= Pose(76.6, 91.0, Math.toRadians(0.0)).mirror()
    //path to pick up PGP motif
    private val pickUpPGP = Pose(127.7, 59.0, Math.toRadians(0.0)).mirror()
    private val pickUpPGPcontrol= Pose(79.29, 64.7, Math.toRadians(0.0)).mirror()
    //path to pick up GPP motif
    private val pickUpGPP = Pose(127.7, 59.0, Math.toRadians(0.0)).mirror()
    private val pickUpGPPcontrol= Pose(82.5, 39.3, Math.toRadians(0.0)).mirror()
    private val pickUPhp= Pose(140.0,8.2,Math.toRadians(0.0)).mirror()
